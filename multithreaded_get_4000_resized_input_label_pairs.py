...
def thread_function(low, high):
	...
	input_label_pairs_file = h5py.File("ga_4000_input_label_pairs_resized_low_%d_high_%d.hdf5" % (low, high), "w")
	...
	for key in list(read_signal_file.keys())[low:high]:
		...
		for i in range (0, num_windows_in_signal):
			...
			read_chanel_instance_group = input_label_pairs_file.create_group(key + "_Instance_" + str(i))	
...
for i in range(0,num_threads):
	values_per_thread = int (num_input_label_pairs / num_threads) + 1
	low = values_per_thread*i
	high = values_per_thread*(i+1)
